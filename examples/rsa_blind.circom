...
template BigIntI2OSP(k, bpl) {
    signal input in[k];
    signal output out[k * bpl];
...
template BigIntOS2IP(k, bpl) {
  signal input in[k*bpl];
  signal output out[k];
  component conv[k];
  for (var i = 0; i < k; i++) {
    conv[i] = OS2IP(bpl);
    for (var j = 0; j < bpl; j++) {
      conv[i].in[j] <== in[i*bpl + j];
    }
    out[k - 1 - i] <== conv[i].out;
  }
}
...
// Blind RSA-PSS prover circuit using SHA-256
template BlindRSAPSS(w, k, eBits, hashLen, saltLen, mgfCount) {
    // TODO MANCA HASH DEL MESSAGGIO
    ...
    signal input salt[saltLen];
    signal input r[k];
    signal output blinded[k*bpl];

    component enc = EMSA_PSS_Encode(emBytes, hashLen, saltLen, mgfCount);
    for (var i = 0; i < hashLen; i++) enc.hashed[i] <== hashed[i];
    for (var i = 0; i < saltLen; i++) enc.salt[i] <== salt[i];

    component pmR = PowerMod(w, k, eBits);
    for (var i = 0; i < k; i++) pmR.base[i] <== r[i];
    for (var i = 0; i < k; i++) pmR.exp[i] <== exp[i];
    for (var i = 0; i < k; i++) pmR.modulus[i] <== modulus[i];

    component ipc = BigIntOS2IP(k,bpl);
    for (var i = 0; i < k; i++) {
        for (var j = 0; j < bpl; j++) ipc.in[i * bpl + j] <== enc.EM[i * bpl + j];
    }

    component mulmod = BigMultModP(w, k);
    for (var i = 0; i < k; i++) {
        mulmod.a[i] <== ipc.out[i];
        mulmod.b[i] <== pmR.out[i];
        mulmod.p[i] <== modulus[i];
    }

    component toBytes = BigIntI2OSP(k, bpl);
    for (var i = 0; i < k; i++) {
        toBytes.in[i] <== mulmod.out[i];
    }


    for (var i = 0; i < k*bpl; i++) {
        blinded[i] <== toBytes.out[i];
    }
}

// Instantiate for a 2048-bit key with SHA-256: mdLen=32, saltLen=32, count=7
// component main { public [exp, modulus, hashed] } = BlindRSAPSS(64, 32, 17, 32, 32, 7);