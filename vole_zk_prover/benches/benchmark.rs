...
fn load_and_prove() {
    let before = memory_stats().map(|s| s.physical_mem).unwrap_or(0);
    ...
    let proof = prover.prove().unwrap();
     ...
     let after = memory_stats().map(|s| s.physical_mem).unwrap_or(0);
    println!("Memory used: {:.2} MB", (after - before) as f64 / 1024.0 / 1024.0);
}
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let ux: Vec<u64> = (0..1024).map(|_|ThreadRng::default()).collect();
    let uy: Vec<u64> = (0..1024).map(|_|ThreadRng::default()).collect();
